...
import { DynamoProjectRepository } from "./infra/aws/dynamo-project-repository.js";
import { DynamoApiKeyRepository } from "./infra/aws/dynamo-api-key-repository.js";
import { DynamoShareLinkRepository } from "./infra/aws/dynamo-share-link-repository.js";
...
export function createContainer(config: ContainerConfig = {}): AppContainer {
  ...
  const projectRepo = config.projectRepo ?? new DynamoProjectRepository(db);
  const apiKeyRepo = config.apiKeyRepo ?? new DynamoApiKeyRepository(db);
  const shareRepo = config.shareRepo ?? new DynamoShareLinkRepository(db);
