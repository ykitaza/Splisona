...
手元に画像ファイルが既にある場合の分割は、必ず専用コマンドを使う:

```bash
splisona split lp.png -o /tmp/segs/A.png    # → A-1.png, A-2.png, ... を生成
```

**sips や ImageMagick 等で分割を自作しないこと**（過去に sips の cropOffset が効かず、
全セグメントが同一の中央部分になる事故が起きた。split コマンドは検証済みの分割ロジックを使う）。
分割後は先頭セグメントがページ最上部（ヒーロー）から始まっていることを Read で目視確認する。
